...
def test_safety_valve_with_6_specialists(termination_fn):
    """Safety valve should fire at n_specialists * 3 + 6 = 24 messages for 6 specialists."""
    # 25 messages should trigger safety valve (> 24)
...
def test_no_premature_termination_under_safety_valve(termination_fn):
    ...
    # 23 messages should NOT trigger safety valve (not > 24)
    conversation = [_msg(f"Message {i}") for i in range(23)]
    assert termination_fn(conversation) is False
